...
根据与老师的进一步确认，本方案冻结以下三项解释：

1. “RL 探索”主要指 RL 继续作为 MPPI sampling prior/trajectory proposer，帮助探索不同候选控制轨迹，而不是让 RL 专门采集 residual 微调数据；
2. 两类降级策略都保留：简单场景减少 RL candidates、更多依靠 traditional MPPI；稳定动力学环境中 residual 默认冻结，只在检测到明确变化且独立验证通过时进行一次受限微调并重新冻结；
3. 不同 residual 方法、RL 轨迹、静态/动态障碍和轨迹评价指标属于实验消融与评价维度，不全部包装成方法创新。

在此边界下，ICODE-style residual model 负责修正 rollout prediction，RL 负责提出候选控制轨迹，MPPI 负责在线评价与控制更新，现有 safety arbitration 继续拥有最终执行权。RL training experience 默认不回流更新 ICODE。

本稿是据此形成的执行版。文中“拟议贡献”“预期关系”和阶段阈值仍需通过相关工作核查、代码审计和正式实验验证，不作为已经获得的论文结论。
...
| ICODE 的部署方式 | residual gating，并考虑 on-policy 更新 | 主实验前冻结；动力学变化时只做一次受限微调并重新冻结 |
| 调节机制 | residual trust 与 policy trust 的双层复杂门控 | RL candidate allocation + residual 冻结/低频微调生命周期；不做持续联合训练 |
...
Residual 的冻结/微调策略作为受控工程问题单独评价：固定环境中不更新；动力学变化时只允许一次低频、有限步数的离线或 shadow adaptation，验证通过后重新冻结。它不与 RL prior 联合训练，也不扩展成持续在线学习主线。
...
#### 4.1.1 Residual 冻结与低频微调策略

根据老师确认，默认部署策略为“训练一次、长期冻结、必要时少量微调、通过验证后重新冻结”，而不是每个控制周期持续学习。

在长度为 $W$ 的监测窗口中记录 one-step transition error：
...
该误差只作为 adaptation request 的诊断信号，不直接在闭环中更新网络。执行流程固定为：

~~~text
稳定环境、误差处于 validation 范围
→ 保持 frozen checkpoint
→ 只做前向推理

误差连续多个窗口超出阈值
→ 进入日志/offline 或 shadow 检查
→ 在无障碍或少障碍低速条件下独立采集 adaptation set
→ 复制 checkpoint，进行一次有上限的 fine-tuning
→ 用独立 validation/holdout 检查 H-step error、闭环性能和 latency
→ 通过后替换并重新冻结；未通过则保留旧 checkpoint
~~~

微调数据不由 RL training experience 提供，微调 optimizer 不与 RL optimizer 联合运行。正式实验固定 adaptation transitions、gradient steps、learning rate 和 wall-clock 上限，并报告：

- frozen 与 one-shot fine-tuned prediction error；
- 所需 adaptation samples；
- 微调 wall-clock；
- 更新前后 planner latency；
- checkpoint 被接受或拒绝的比例；
- 固定环境中“不微调”的计算节省。

该策略属于计算效率和部署消融，不单独包装为主要算法创新。
...
RL 轨迹可以记录并分析其状态—控制覆盖，但默认不进入 residual Train。确认保留的一次 fine-tuning 消融使用独立的预设控制激励采集 adaptation split，不使用 RL training experience，并重新保留未触碰的 final test。若另行研究“使用 RL 日志微调”，只能作为非默认敏感性分析单独命名和报告。
...
| One-shot adaptation | 500 transitions | 2000 transitions、1000 gradient steps 或 30 min | 任一上限先达到即停止 |
...
Residual 生命周期消融进一步比较 frozen 与 one-shot fine-tuned checkpoint。新 checkpoint 只有同时满足以下条件才被接受：

- dynamics-shift holdout 上 $H=10/20$ RMSE 相对 frozen checkpoint 至少降低 10%；
- 原 IID holdout 的 RMSE 退化不超过 5%；
- planner p95 latency 仍满足 G5；
- 不增加 collision/safety intervention 的明显风险。

未通过时继续使用旧 frozen checkpoint，并将该次 adaptation 记录为 rejected update。
...
根据老师确认，本章中的四类内容——不同 residual 方法、RL/traditional 规划轨迹、静态/动态障碍以及评价指标——属于**实验消融与评价维度**。它们用于回答每个模块是否有效、在哪些条件下有效，不分别包装成四项算法创新。
...
- one-shot fine-tuned ICODE：用于确认 residual 在稳定环境冻结、动力学变化时低频微调的部署策略；
...
Frozen vs one-shot fine-tuned ICODE:
2 methods × 2 clean/simple conditions × 5 environment seeds
= 20 episodes
...
按上述固定复用规则，A–E 为 530 个不重复 episodes；加上 F 的必要诊断后，正式 MuJoCo 计划固定为 590 episodes。G 的仿真扩展上限为 55 episodes，因此仿真总上限仍为 645 episodes。先使用 development seeds 淘汰明显无效实现，再运行冻结后的 final test seeds。
...
**任务：** G0 通过后采集正式 residual 数据，训练 nominal/MLP/ICODE，完成 H-step prediction benchmark、inference 优化及 frozen-vs-one-shot-fine-tuned 生命周期消融。  
**验收：** Oracle 接口成立；ICODE 在主要 validation 条件改善 H=10/20；checkpoint、normalizer 和数据版本冻结；微调只有在独立 holdout 改善时才被接受并重新冻结。  
...
| G1 Residual（07-27） | H=10/20 RMSE 至少改善 15%；dev collision 不增加超过 2 percentage points；可实时推理 | 不做联合主张，汇报结果并按预案收敛为 RL-guided nominal MPPI 或重新审计 residual 数据链 |
...
| 07-26 至 08-06 | 实现双源候选集、fixed $\eta$ 和候选质量指标 | 核心 2×2 development seeds；并行完成 frozen-vs-one-shot-fine-tuned 小型消融 | 完成 Ablation/Metric 协议 |
...
1. $K=25$、$K=200$ 补充点；
2. `lab_complex` 等非核心场景；
3. 动态障碍扩展；
4. 实车闭环统计，保留 offline/shadow；
5. MLP/Oracle 的完整闭环，只保留 prediction；
6. 额外 uncertainty detector；
7. 非核心 seeds 的宽矩阵。
...
- 最小 frozen-vs-one-shot-fine-tuned residual 生命周期消融；
...
1. RL 保留为 MPPI sampling prior/trajectory proposer，探索对象是候选控制轨迹空间；
2. RL 不直接发布最终控制，也不作为默认 residual 数据采集器；
3. 简单场景保留 traditional MPPI 优势，通过双源候选分配减少不必要的 RL 参与；
4. Residual 在固定环境默认冻结，动力学发生明确变化时只做一次受限微调，验证后重新冻结；
5. 不同 residual、RL/traditional 轨迹、静态/动态障碍和指标属于实验消融与评价；
6. 静态障碍承担主要闭环结论，动态障碍只作条件性、反应式扩展；
7. 论文不设置形式化定理或命题，不声称稳定性、安全性或任意 OOD 保证；
8. 核心创新候选集中在 source-relative adaptive allocation，是否成立由相关工作 Gate 和消融共同决定。

### 13.2 接下来立即执行的顺序

~~~text
第一步：冻结 Strong Traditional MPPI 与 MuJoCo 物理/计时基线
    ↓
第二步：完成 Formal Frozen ICODE 与 one-shot fine-tune 小型消融
    ↓
第三步：固定 ICODE checkpoint 后训练一种 RL trajectory proposer
    ↓
第四步：实现 traditional/RL 双源 candidate batches
    ↓
第五步：先验证 fixed eta，再验证 source-relative adaptive eta
    ↓
第六步：完成 2×2、RL learning curve、等 samples/等 wall-clock
    ↓
第七步：在核心结果通过后再执行动态障碍和低速实车扩展
~~~

任何 Gate 失败时，先解释失败原因并收缩主张，不通过继续增加新网络、场景或理论元素补救。所有工作仍以 08-10 方法硬冻结、09-14 北京时间内部提交为固定边界。
