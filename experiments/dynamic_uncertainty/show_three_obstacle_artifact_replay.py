"""Replay a saved three-obstacle MuJoCo artifact without replanning.

This utility is visualization-only.  It re-executes the recorded control
commands against the frozen MuJoCo plant and obstacle seed.  It never invokes
the controller and never writes experimental metrics, so the replay must not
be treated as a new experimental result.
"""
...
DEFAULT_ARTIFACT = (
    ROOT
    / "research_artifacts"
    / "dynamic_actor_three_obstacle_amendment32_v5_seed730100252"
)


def _load_controls(trajectory_path: Path):
    with trajectory_path.open("r", encoding="utf-8", newline="") as stream:
        rows = list(csv.DictReader(stream))
    if not rows:
        raise ValueError(f"empty saved trajectory: {trajectory_path}")
    return tuple(
        (
            float(row["executed_v"]),
            float(row["executed_omega"]),
        )
        for row in rows
    )


def replay(artifact: Path, arm: str, playback_speed: float, loop: bool):
    run_dir = artifact.resolve() / "runs" / arm
    config_path = run_dir / "config_resolved.yaml"
    trajectory_path = run_dir / "trajectory.csv"
    if not config_path.is_file() or not trajectory_path.is_file():
        raise FileNotFoundError(
            f"saved run is incomplete: expected {config_path} and {trajectory_path}"
        )

    config = load_yaml(config_path)
    if len(config.get("scene", {}).get("obstacles", ())) != 3:
        raise ValueError("artifact is not a three-obstacle scene")
    if str(config.get("plant", {}).get("backend")) != "mujoco_diff_drive":
        raise ValueError("artifact is not backed by the MuJoCo differential-drive plant")

    controls = _load_controls(trajectory_path)
...
def main(argv=None):
    ...
    parser.add_argument(
        "--arm",
        choices=("dynamic_actor", "source_actor"),
        default="dynamic_actor",
    )
    ...
    replay(args.artifact, args.arm, args.playback_speed, bool(args.loop))
    return 0
