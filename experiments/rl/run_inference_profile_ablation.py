#!/usr/bin/env python3
"""Run one checkpoint block of the preregistered L28 inference ablation."""
...
PROFILE_FIELDS = STEP_FIELDS + (
    "rl_selected_consensus_lcb",
    "rl_target_q1_base",
    "rl_target_q2_base",
    "rl_target_q1_candidate",
    ...
    "profile_prior_fallback_ms",
    "profile_prior_encode_normalize_ms",
    "profile_prior_gate_features_ms",
    "profile_prior_actor_ms",
    "profile_prior_advantage_ms",
    "profile_prior_decoder_ms",
    "profile_prior_outer_gate_ms",
    "profile_prior_total_ms",
)


def _resolved_condition(
    base, scene_path, condition, checkpoint, seed, num_samples
):
    config = _condition_config(
        base, scene_path, "complexity_lcb", checkpoint, seed
    )
    config["planner"]["num_samples"] = int(num_samples)
    config["planner"]["profile_components"] = True
    gate = config["rl"]["gate"]
    gate["skip_zero_complexity_inference"] = True
    gate["profile_inference"] = True
    gate["compute_unselected_critic_diagnostics"] = bool(
        condition == "reference_full_diagnostics"
    )
    gate["reuse_policy_base_action"] = bool(
        condition == "selected_critic_base_reuse"
    )
    config["experiment"]["name"] = "l28_%s_seed_%d" % (
        condition, int(seed)
    )
    return config


def _protected_seeds(base):
    rl = base["rl"]
    protected = set()
    for section, keys in (
        ("scene_complexity_ablation", ("sealed_test_episode_seeds",)),
        (
            "sample_efficiency_ablation",
            ("forbidden_l25_seeds", "sealed_test_episode_seeds"),
        ),
        ("zero_complexity_fastpath_ablation", ("sealed_test_episode_seeds",)),
        ("inference_profile_ablation", ("sealed_test_episode_seeds",)),
    ):
        for key in keys:
            protected.update(
                int(value) for value in rl[section].get(key, [])
            )
    return protected
...
def main(argv=None):
    ...
    if not seeds or len(seeds) != len(set(seeds)):
        raise ValueError("L28 seeds must be nonempty and unique")
    if tuple(design["conditions"]) != CONDITIONS:
        raise ValueError("L28 condition order differs from preregistration")
    protected = _protected_seeds(base)
    if protected.intersection(seeds) and not args.allow_sealed_test:
        raise ValueError("sealed L25/L26/L27/L28 seeds require explicit override")
    ...
    for index, item in enumerate(schedule):
        ...
        run_dir = (
            output / "runs" / scene["name"] / condition /
            ("seed_%d" % seed)
        )
