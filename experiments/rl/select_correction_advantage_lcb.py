#!/usr/bin/env python3
"""Audit multi-training-seed LCB grids and freeze one global decision."""
...
def main(argv=None):
    ...
    if len(run_dirs) < 2:
        raise ValueError("LCB selection requires multiple training seeds")

    config = CriticConsensusSelectionConfig()
    config.validate()
    expected_betas = [float(value) for value in config.candidate_betas]
    ...
    for run_dir in run_dirs:
        ...
        betas = [float(value) for value in summary["betas"]]
        if betas != expected_betas:
            raise ValueError("run beta grid differs from preregistration")
        mapping = {
            str(name): float(value)
            for name, value in summary["condition_beta"].items()
        }
        if sorted(mapping.values()) != expected_betas:
            raise ValueError("run condition-to-beta mapping is incomplete")
        if not condition_beta:
            condition_beta = mapping
            expected_conditions.extend(sorted(mapping))
        elif mapping != condition_beta:
            raise ValueError("runs use different condition-to-beta mappings")
        ...
        run_quality, paired = _run_quality(
            ...
            expected_conditions,
            episode_seeds,
            extra_critical_step_fields=LCB_CRITICAL_FIELDS,
        )
    ...
    candidates = {}
    for condition, beta in condition_beta.items():
        candidates[beta] = {
    ...
    selection = select_critic_consensus_beta(candidates, config)
    quality_passed = all(
    ...
    result = {
        ...
        "per_training_seed": {
            ...
            for condition in sorted(condition_beta)
        },
        "selection": selection,
