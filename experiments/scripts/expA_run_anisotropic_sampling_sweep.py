...
SWEEP_CONFIGS = {
    ...
    "dense_sampling_sdf_distance_compare_large_h": {
        "grid_mode": "product",
        "grid": {
            "scene_name": ["dense"],
            "horizon": [30, 35],
            "sdf_influence_distance": [0.9, 1.2, 1.5],
        },
        "fixed": {
            "num_samples": 250,
            "temperature": 8.0,
        ...
        "table_filename": "dense_sampling_sdf_distance_compare_large_h_summary.csv",
    },
}


ACTIVE_SWEEP_NAME = "dense_sampling_sdf_distance_compare_large_h"
...
def build_experiment_cases(base_config, sweep_config):
    ...
    if grid_mode == "product":
        for grid_values in product(*grid_value_lists):
            ...
            case.update(fixed)
            case.update(dict(zip(grid_keys, grid_values)))
            experiment_cases.append(case)
    elif grid_mode == "zip":
        lengths = [len(v) for v in grid_value_lists]
        if len(set(lengths)) != 1:
            raise ValueError(f"zip mode requires all grid lists to have same length, got lengths={lengths}")
        ...
        raise ValueError(f"Unknown grid_mode: {grid_mode}. Expected 'product' or 'zip'.")
...
def print_brief_summary(all_results):
    ...
    for row in all_results:
        print(
            ...
            f'sdf_dist={row.get("sdf_influence_distance")} | '
...
def main():
    if ACTIVE_SWEEP_NAME not in SWEEP_CONFIGS:
        raise ValueError(
            f"Unknown ACTIVE_SWEEP_NAME: {ACTIVE_SWEEP_NAME}. Available options: {list(SWEEP_CONFIGS.keys())}"
        )

    sweep_config = SWEEP_CONFIGS[ACTIVE_SWEEP_NAME]
    experiment_cases = build_experiment_cases(BASE_EXPERIMENT_CONFIG, sweep_config)
