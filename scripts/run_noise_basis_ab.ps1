<#
.SYNOPSIS
  Run the frozen noise-basis A/B development screen (12 paired seeds, 24 episodes).
...
.DESCRIPTION
  ...
  Protocol: configs/research/noise_basis_ab_development_v1.yaml
  Evidence : research_artifacts/noise_basis_ab_v1/

.EXAMPLE
  .\scripts\run_noise_basis_ab.ps1
  .\scripts\run_noise_basis_ab.ps1 -Resume        # skip completed episodes
  .\scripts\run_noise_basis_ab.ps1 -AnalyzeOnly   # just re-apply the decision rule
#>
...
$root    = Join-Path $repo 'research_artifacts\noise_basis_ab_v1'
$mapName = 'chapter1'
$seeds   = 791101201..791101212
...
& $python -m experiments.dynamic_uncertainty.analyze_noise_basis_ab_development --root $root
