...
def test_idle_semantic_stack_quarantines_legacy_escape_before_admission():
    context = EncounterControlAuthority(
        EncounterControlConfig(enabled=True)
    ).planning_context(_intent(encounter_phase="idle"))
...
def test_incomplete_frontal_bypass_commits_to_locked_side_early():
    ...
    result = authority.apply(
        _plan(control=(0.32, -0.14)),
        _intent(
            encounter_phase="frontal_approach",
            encounter_strategy="left_bypass",
            encounter_locked_steering_side=1,
            encounter_entry_goal_heading_rad=0.0,
            encounter_temporary_waypoint=(3.6, 0.9),
        ),
        pose=(0.0, 0.0, 0.30),
        guard_result={"emergency_stop": False, "min_rear_range": None},
    )
